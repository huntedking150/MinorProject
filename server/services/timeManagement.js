...
class MDPSystem {
  ...
  isSlotValidForOrder(order, slot) {
    return (
      this.convertToMinutes(slot.start) >=
      this.convertToMinutes(order.orderTime)
    );
  }
  ...
  async getSlotCapacity() {
    ...
    orders.forEach((order) => {
      if (order.assignedSlot && order.assignedSlot !== 'Rejected') {
        slotCapacity.set(
          order.assignedSlot,
          (slotCapacity.get(order.assignedSlot) || 0) + 1
        );
      }
    });
  ...
  async calculatePriority(order) {
    ...
    const ps =
      ...
      value * weights.w2 -
      urgency * weights.w3 -
      this.convertToMinutes(order.orderTime) * weights.w4;
    console.log(ps);
  ...
  findOptimalSlot(order) {
    ...
    const validSlots = slotsArray.filter((slot) => order.orderTime < slot.end);
    1;

    // Sort slots based on nearest
    validSlots.sort(
      (a, b) => Math.abs(order.time - a.start) - Math.abs(order.time - b.start)
    );
  ...
  findAlternativeSlot(order, optimalSlot) {
    ...
    const validSlots = slotsArray.filter(
      (slot) =>
        order.orderTime < slot.end &&
        !(slot.start === optimalSlot.start && slot.end === optimalSlot.end)
    );

    // Sort by the nearest available slot
    validSlots.sort(
      (a, b) =>
        Math.abs(order.orderTime - a.start) -
        Math.abs(order.orderTime - b.start)
    );
    console.log(validSlots[0]);
  ...
  //     const existingOrders = await find({});
  ...
  //         await updateOne(
  //           { _id: newOrder._id },
  //           { assignedSlot: slotKey, priority: newOrder.priority }
  //         );

  //         // Update slot capacity
  //         slotCapacity.set(slotKey, (slotCapacity.get(slotKey) || 0) + 1);

  //         let conflictingOrders = existingOrders.filter(
  //           (o) => o.assignedSlot === slotKey
  //         );
  ...
  //               slotCapacity.set(
  //                 lowestPriorityOrder.assignedSlot,
  //                 (slotCapacity.get(lowestPriorityOrder.assignedSlot) || 0) + 1
  //               );
  //             } else {
  ...
  //             await updateOne(
  //               { _id: lowestPriorityOrder._id },
  //               { assignedSlot: lowestPriorityOrder.assignedSlot }
  //             );
  ...
  //         console.log(
  //           `New order ${newOrder._id} allocated to ${slotKey}, lower priority orders were adjusted.`
  //         );
  //       } else {
  //         newOrder.assignedSlot = 'Rejected';
  //         await updateOne({ _id: newOrder._id }, { assignedSlot: 'Rejected' });
  //         console.log(
  //           `New order ${newOrder._id} rejected due to no available slots.`
  //         );
  //       }
  //     }
  //   }
  ...
  async allocateOrders(newOrder) {
    ...
    if (newOrder) {
      newOrder.priority = await this.calculatePriority(newOrder);
      const optimalSlot = this.findOptimalSlot(newOrder);
      ...
      if (optimalSlot) {
        ...
        if (slotCapacity.get(slotKey) > MAX_ORDERS_PER_SLOT) {
          conflictingOrders.sort((a, b) => a.priority - b.priority); // Sort by priority

          while (slotCapacity.get(slotKey) > MAX_ORDERS_PER_SLOT) {
            const lowestPriorityOrder = conflictingOrders.shift();
            const newSlot = this.findAlternativeSlot(
              lowestPriorityOrder,
              optimalSlot
            );
...
export async function getNextAvailableTimeSlot(adminId) {
  const mdpSystem = new MDPSystem(timeSlots, MAX_ORDERS_PER_SLOT);
  return mdpSystem.findOptimalSlot({ orderTime: '8:30 AM' }); // Dummy example, replace with actual logic
}

export default MDPSystem;
